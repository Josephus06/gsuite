...
router.get('/', requireAuth, requirePermission(ROUTE, 'can_view'), async (req, res, next) => {
  try {
    const [rows] = await pool.query(
      'SELECT account_type, page_id, can_view, can_add, can_edit, can_view_all, can_delete, can_approve, can_print FROM account_type_permissions'
    );
    const byType = {};
    rows.forEach((r) => {
...
router.put('/:accountType', requireAuth, requirePermission(ROUTE, 'can_edit'), async (req, res, next) => {
  ...
  try {
    ...
    for (const p of permissions) {
      ...
      await conn.query(
        `INSERT INTO account_type_permissions
           (account_type, page_id, can_view, can_add, can_edit, can_view_all, can_delete, can_approve, can_print, updated_at, updated_by_user_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)`,
    ...
    const [rows] = await pool.query(
      'SELECT account_type, page_id, can_view, can_add, can_edit, can_view_all, can_delete, can_approve, can_print FROM account_type_permissions WHERE account_type = ?',
      [accountType]
    );
    res.json(rows);
  } catch (err) {
