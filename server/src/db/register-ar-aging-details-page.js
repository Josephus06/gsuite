// One-off migration: registers Accounting > Reports > AR Aging Details in the `pages` table.
//
// requirePermission resolves a route to a `pages` row before it checks anything, so without this
// row /reports/ar-aging-details refuses every user including System Admin, and the nav hides the
...
// what turns a missing row into a 500 instead of a 403, and it takes away the ability to grant
// one report without the other.
//
// No schema change and no index: the report reads the same three tables AR Aging already reads,
// through the same queries.
//
// Idempotent -- safe to re-run:
//   node src/db/register-ar-aging-details-page.js --dry-run   (report only)
//   node src/db/register-ar-aging-details-page.js             (apply)
require('dotenv').config();
const pool = require('../db');

const DRY_RUN = process.argv.includes('--dry-run');
const ROUTE = '/reports/ar-aging-details';
const NAME = 'AR Aging Details';
